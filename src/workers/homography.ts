...
export function applyHomographicTransform(
  ...
  for (let y = 0; y < size; y++) {
    ...
    for (let x = 0; x < size; x++) {
      ...
      result[y * size + x] = source.bytes[sy * source.width + sx];
    }
  }
  return result;
}

export function createGridLines(transform: Transform, size: number) {
  ...
  for (let l = 1; l < 9; l++) {
    ...
    gridLines.push({
      ...
      p2: transformPoint({ x: size, y: l * boxSize }, transform),
    });
    // vertical line
    gridLines.push({
      p1: transformPoint({ y: 0, x: l * boxSize }, transform),
      p2: transformPoint({ y: size, x: l * boxSize }, transform),
    });
  }
  return gridLines;
}
